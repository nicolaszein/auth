...
class Token:
    ...
    def generate_token(self, user_id, session_id, expire_in=TOKEN_EXPIRATION_TIME):
        claims_data = self.__build_claims_data(user_id, session_id, expire_in)
    ...
    def generate_refresh_token(self, user_id, session_id):
        claims_data = self.__build_claims_data(user_id, session_id)

        token = self.__build_token(claims_data)

        return token

    def __build_claims_data(self, user_id, session_id, expire_in=None):
