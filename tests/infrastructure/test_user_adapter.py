...
def test_create(user_repository_mock, user_entity_mock):
    user = User(
        full_name="Foo Bar", email="foo.bar@email.com", password="hashed_password"
    )
...
def test_fetch_by_email(user_repository_mock):
    id = uuid.uuid4()
    email = "foo.bar@email.com"
    user = User(id=id, full_name="Foo Bar", email=email, password="hashed_password")
    user_entity = MagicMock()
...
def test_fetch_by_reset_password_token(user_repository_mock):
    ...
    result = UserAdapter().fetch_by_reset_password_token(
...
def test_fetch_by_reset_password_token_with_user_not_found(user_repository_mock):
    ...
    with pytest.raises(UserNotFound):
        UserAdapter().fetch_by_reset_password_token(
            reset_password_token=reset_password_token
        )
...
def test_send_activation_email(sendgrid_client_mock):
    code = uuid.uuid4()
    user_id = uuid.uuid4()
    call_to_action = f"{AUTH_APP_URL}/activate?code={code}"
    user = User(
        id=user_id,
        full_name="Foo Bar",
        email="foo.bar@email.com",
