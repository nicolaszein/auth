...
def test_send_template_message_build_mail_instance(mail_mock, _):
    ...
    mail_mock.assert_called_once_with(from_email='no-reply@nicolaszein.dev', to_emails=to)
    assert mail_mock().template_id == template_id
    assert mail_mock().dynamic_template_data == {**template_data, **{'subject': subject}}
